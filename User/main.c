...
int main() {
	...
	// Open the GPU name registry key
	RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}"), 0, KEY_ALL_ACCESS, &hKey);

		// Change the dedicated and internal GPU names to something random
	RegEnumValue(hKey, 0, NULL, NULL, NULL, NULL, NULL, NULL);
			printf("changed?");

			RegQueryValueEx(hKey, TEXT("GPU Description"), NULL, &dwType, (LPBYTE)&dwValue, &dwSize);
				char randomNamea[10];
			srand(time(NULL));
			for (int i = 0; i < 10; i++) {
				randomNamea[i] = (char)(rand() % 26 + 97);
			}
			RegSetValueEx(hKey, TEXT("GPU Description"), NULL, REG_SZ, (LPBYTE)randomNamea, strlen(randomNamea) * sizeof(char));
		
		printf("changed?");
		system("pause");
	// Close the registry key
	RegCloseKey(hKey);
	// Open the baseboard manufacturer name registry key
	RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("HARDWARE\\DESCRIPTION\\System\\BIOS"), 0, KEY_ALL_ACCESS, &hKey);

		// Change the baseboard manufacturer name to something random
		char randomName[10];
	srand(time(NULL));
	for (int i = 0; i < 10; i++) {
		randomName[i] = (char)(rand() % 26 + 97);
	}
	RegSetValueEx(hKey, TEXT("BaseBoardManufacturer"), NULL, REG_SZ, (LPBYTE)randomName, strlen(randomName) * sizeof(char));
	...
	// read the value
	if (RegQueryValueEx(hKey, L"ProcessorNameString", 0, &dwType,
	...
	// write the new value
	if (RegSetValueEx(hKey, L"ProcessorNameString", 0, dwType,
